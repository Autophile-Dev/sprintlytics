<template>
  <div class="kpis-analytics-page">

    <!-- ── Global Filters Topbar ── -->
    <header class="va-topbar">
      <div class="topbar-left">
        <div class="title-with-badge">
          <h1 class="page-main-title">Team Performance KPIs</h1>
        ...
        <p class="page-main-subtitle">Engineering cycle time, delivery predictability, code quality, PR throughput &amp; AI benchmarks</p>
      </div>
    ...
    <div v-else-if="!data && !pending" class="empty-state">
      ...
      <h2>No Performance KPI Data Found</h2>
      <p>No KPI analytics records found for <strong>{{ selectedProject }}</strong>.</p>
    </div>
    ...
    <template v-else>
      ...
      <section class="section-container">
        <div class="section-header">
          <div class="sh-title-wrap">
            <h2 class="section-title">KPI Executive Overview</h2>
            <span class="sec-subtitle">Strategic engineering metrics and productivity benchmarks vs previous period</span>
          </div>
        </div>
            ...
            <div class="kpi-header-row">
              <div class="kpi-title-with-icon">
                <div class="kpi-icon-badge" :class="kpi.variant">
                  <svg v-if="key === 'velocityPredictability'" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>
                  <svg v-else-if="key === 'averageCycleTime'" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
                  <svg v-else-if="key === 'sprintDeliveryRate'" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>
                  <svg v-else-if="key === 'defectDensity'" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="m21.73 18-8-14a2 2 0 0 0-3.48 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3Z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>
                  <svg v-else-if="key === 'reviewTurnaround'" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/></svg>
                  <svg v-else-if="key === 'codeReworkRatio'" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="23 4 23 10 17 10"/><path d="M20.49 15a9 9 0 1 1-2.12-9.36L23 10"/></svg>
                  <svg v-else-if="key === 'slaCompliance'" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/><polyline points="9 12 11 14 15 10"/></svg>
      ...
      <!-- ══════════════════════════════════════════════ -->
      <!-- SECTION 2 — KPI Performance Trend Analysis     -->
      <!-- ══════════════════════════════════════════════ -->
      <section class="section-container">
        <div class="card-analytics">
          <div class="card-analytics-header">
            <div>
              <h3 class="card-title">Cycle Time &amp; Delivery Predictability Trend</h3>
              <p class="card-desc">Sprint-over-sprint cycle time (days) vs target commitment predictability %</p>
            </div>
            <div class="chart-controls">
              <button
                class="btn-toggle-sm"
                :class="{ active: showSlaBand }"
                @click="showSlaBand = !showSlaBand"
              >
                Show 90% Predictability Target Band
              </button>
            </div>
          </div>
          ...
          <div class="line-chart-area" @mousemove="handleTrendChartHover" @mouseleave="hoveredTrendIdx = null">
            <svg :viewBox="`0 0 ${chartWidth} ${chartHeight}`" class="trend-svg">
              <defs>
                <linearGradient id="purpleKpiGrad" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="0%" stop-color="#8B5CF6" stop-opacity="0.25"/>
                  <stop offset="100%" stop-color="#8B5CF6" stop-opacity="0.00"/>
                </linearGradient>
                <linearGradient id="slaTargetBandGrad" x1="0" y1="0" x2="0" y2="1">
              ...
              <rect
                ...
                fill="url(#slaTargetBandGrad)"
                stroke="#A7F3D0"
                stroke-dasharray="3,3"
              />

              <!-- Target 90% Line (Emerald Dashed) -->
              <line :x1="margin.left" :y1="target90Y" :x2="chartWidth - margin.right" :y2="target90Y" stroke="#059669" stroke-width="1.8" stroke-dasharray="5,4"/>

              <!-- Shaded area under predictability line -->
              <path :d="predictabilityAreaPath" fill="url(#purpleKpiGrad)" />

              <!-- Predictability Line (Purple Solid) -->
              <path :d="predictabilityLinePath" fill="none" stroke="#8B5CF6" stroke-width="3.5" stroke-linecap="round"/>

              <!-- Cycle Time Line (Blue Dashed) -->
              <path :d="cycleTimeLinePath" fill="none" stroke="#2563EB" stroke-width="2.5" stroke-dasharray="6,4" stroke-linecap="round"/>
              ...
              <g v-if="hoveredTrendIdx !== null && trendPoints[hoveredTrendIdx]">
                ...
                />
                <circle :cx="trendPoints[hoveredTrendIdx].x" :cy="trendPoints[hoveredTrendIdx].predY" r="6" fill="#8B5CF6" stroke="#ffffff" stroke-width="2.5"/>
                <circle :cx="trendPoints[hoveredTrendIdx].x" :cy="trendPoints[hoveredTrendIdx].cycleY" r="5" fill="#2563EB" stroke="#ffffff" stroke-width="2"/>
              </g>
            ...
            >
              ...
              <div class="tooltip-row purple">
                <span class="dot"></span>
                <span>Velocity Predictability: <strong>{{ trendPoints[hoveredTrendIdx].predictabilityPct }}%</strong></span>
              </div>
              <div class="tooltip-row blue">
                <span class="dot"></span>
                <span>Avg Cycle Time: <strong>{{ trendPoints[hoveredTrendIdx].cycleTimeDays }} days</strong></span>
              </div>
              <div class="tooltip-row emerald">
                <span class="dot"></span>
                <span>PRs Merged: <strong>{{ trendPoints[hoveredTrendIdx].prsMerged }} PRs</strong></span>
              </div>
            </div>
          </div>
          ...
          <div class="chart-legend-row">
            <div class="leg-item"><span class="leg-color-dot purple"></span><span>Velocity Predictability %</span></div>
            <div class="leg-item"><span class="leg-color-dot blue-dashed"></span><span>Cycle Time (days)</span></div>
            <div class="leg-item"><span class="leg-color-dot emerald-dashed"></span><span>90% SLA Target Benchmark</span></div>
          </div>
        </div>
      </section>
      ...
      <section class="section-container dual-grid">
        ...
        <!-- SECTION 3 — Strategic Engineering KPI Pillars -->
        <div class="card-analytics">
          <div class="card-analytics-header">
            <div>
              <h3 class="card-title">Strategic Engineering KPI Pillars</h3>
              <p class="card-desc">Core engineering performance ratings across speed, quality, predictability &amp; reviews</p>
            </div>
          </div>

          <div class="role-bars-wrapper">
            <div v-for="p in kpiPillars" :key="p.name" class="role-bar-card">
              <div class="rb-header">
                <span class="rb-title">{{ p.name }}</span>
                <span class="rb-val">{{ p.score }}% ({{ p.status }})</span>
              </div>
          ...
          <div class="chart-legend-row center">
            <div class="leg-item"><span class="leg-color-dot emerald"></span><span>Target SLA ≥ 90%</span></div>
            <div class="leg-item"><span class="leg-color-dot blue"></span><span>Target Quality ≥ 92%</span></div>
          </div>
        </div>

        <!-- SECTION 4 — Cycle Time Phase Breakdown -->
        <div class="card-analytics">
          <div class="card-analytics-header">
            <div>
              <h3 class="card-title">Cycle Time Phase Distribution</h3>
              <p class="card-desc">Engineering task duration breakdown from coding to deployment</p>
            </div>
            <span class="consistency-score-badge">
              Avg Total: {{ executiveKpis.averageCycleTime ? executiveKpis.averageCycleTime.value : '1.8d' }}
            </span>
          </div>

          <div class="phase-breakdown-wrapper">
            <!-- Stacked Multi-color Phase Bar -->
            <div class="stacked-cap-bar">
              <div
                v-for="ph in cycleTimePhases"
                :key="'ph-'+ph.phase"
                class="stacked-seg"
                :style="{ width: ph.pct + '%', backgroundColor: ph.color }"
                :title="ph.phase + ': ' + ph.days + ' days (' + ph.pct + '%)'"
              ></div>
            </div>

            <div class="work-cat-grid">
              <div v-for="ph in cycleTimePhases" :key="'phc-'+ph.phase" class="work-cat-card">
                <div class="wcc-header">
                  <span class="dist-dot" :style="{ backgroundColor: ph.color }"></span>
                  <span class="wcc-name">{{ ph.phase }}</span>
                </div>
                <div class="wcc-body">
                  <strong class="wcc-pct">{{ ph.days }} days</strong>
                  <span class="wcc-pts">{{ ph.pct }}% of cycle</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="consistency-metrics-row">
            <div class="c-metric-pill"><span>Coding:</span> <strong>{{ cycleTimePhases[0] ? cycleTimePhases[0].days : '0.8' }}d</strong></div>
            <div class="c-metric-pill"><span>Review:</span> <strong>{{ cycleTimePhases[1] ? cycleTimePhases[1].days : '0.4' }}d</strong></div>
            <div class="c-metric-pill"><span>QA Test:</span> <strong>{{ cycleTimePhases[2] ? cycleTimePhases[2].days : '0.4' }}d</strong></div>
            <div class="c-metric-pill emerald"><span>Deploy:</span> <strong>{{ cycleTimePhases[3] ? cycleTimePhases[3].days : '0.2' }}d</strong></div>
          </div>
        </div>
      ...
      <!-- ══════════════════════════════════════════════ -->
      <!-- SECTION 5 — Individual Engineer Performance Cards -->
      <!-- ══════════════════════════════════════════════ -->
      <section class="section-container">
        <div class="section-header">
          <div class="sh-title-wrap">
            <h2 class="section-title">Engineer KPI Performance Cards</h2>
            <span class="sec-subtitle">Individual engineer velocity, cycle time, PR throughput &amp; quality scores</span>
          </div>
        </div>
        ...
        <div class="team-cards-grid">
          <div v-for="member in members" :key="'tm-'+member.email" class="team-card">
            <div class="tc-header">
              ...
              <span class="tc-status-badge grade-pill" :class="getGradeClass(member.grade)">
                {{ member.grade }}
              </span>
            </div>
            ...
            <div class="tc-stats-row">
              <div class="tc-stat">
                ...
                <span class="val blue">{{ member.cycleTimeDays }}d</span>
              </div>
              <div class="tc-stat">
                <span class="label">PRs Merged</span>
                <span class="val emerald">{{ member.prsMerged }} PRs</span>
              </div>
              <div class="tc-stat">
                <span class="label">KPI Score</span>
                <span class="val purple">{{ member.kpiScorePct }}%</span>
              </div>
            </div>
            ...
            <div class="tc-bars-section">
              <div class="tc-bar-group">
                <div class="tc-bar-label"><span>Code Quality Rating</span><strong>{{ member.qualityScore }}%</strong></div>
                <div class="progress-bar-track-sm"><div class="progress-bar-fill-sm emerald" :style="{ width: member.qualityScore + '%' }"></div></div>
              </div>
              <div class="tc-bar-group">
                <div class="tc-bar-label"><span>Review Speed ({{ member.reviewSpeedHours }}h)</span><strong>{{ Math.max(10, Math.min(100, Math.round((6.0 - member.reviewSpeedHours) * 20))) }}%</strong></div>
                <div class="progress-bar-track-sm"><div class="progress-bar-fill-sm blue" :style="{ width: Math.max(10, Math.min(100, Math.round((6.0 - member.reviewSpeedHours) * 20))) + '%' }"></div></div>
              </div>
            </div>

            <!-- Mini Sparkline Row -->
            <div class="tc-sparkline-row">
              <span class="sparkline-label">KPI Score Trend:</span>
      ...
      <!-- ══════════════════════════════════════════════ -->
      <!-- SECTION 6 — Granular Engineering KPI Metrics   -->
      <!-- ══════════════════════════════════════════════ -->
      <section class="section-container">
        <div class="section-header">
          <div class="sh-title-wrap">
            <h2 class="section-title">Granular Engineering Metrics</h2>
            <span class="sec-subtitle">Lead time, CI/CD pass rate, PR approval speed &amp; rework analytics</span>
          </div>
        </div>

        <div class="sp-analytics-grid">
          <div v-for="(gk, gkey) in granularKpis" :key="gkey" class="sp-card">
            <div class="sp-icon-row">
              <div class="kpi-icon-badge blue">
                <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>
              </div>
              <span class="sp-title">{{ getGranularTitle(gkey) }}</span>
            </div>
            <div class="sp-main-val">{{ gk.val }}</div>
            <SegmentedProgressBar :value="gk.pct" variant="emerald" height="14px"/>
            <span class="sp-sub-text">{{ gk.sub }}</span>
          </div>
        </div>
      </section>

      <!-- ══════════════════════════════════════════════ -->
      <!-- SECTION 7 — Team KPI Data Table & Roster      -->
      <!-- ══════════════════════════════════════════════ -->
      <section class="section-container">
        <div class="card-analytics">
          <div class="card-analytics-header">
            <div>
              <h3 class="card-title">Team KPI Roster &amp; Productivity Data</h3>
              <p class="card-desc">Detailed engineer KPI metrics, review speeds, defect density &amp; rating grades</p>
            </div>
            ...
            <table class="velocity-data-table">
              <thead>
                <tr>
                  ...
                  <th @click="sortTable('cycleTimeDays')">Cycle Time {{ getSortIcon('cycleTimeDays') }}</th>
                  <th @click="sortTable('prsMerged')">PRs Merged {{ getSortIcon('prsMerged') }}</th>
                  <th @click="sortTable('qualityScore')">Quality Score {{ getSortIcon('qualityScore') }}</th>
                  <th @click="sortTable('kpiScorePct')">KPI Score % {{ getSortIcon('kpiScorePct') }}</th>
                  <th>Grade Rating</th>
                </tr>
              </thead>
              <tbody>
                <tr v-for="row in filteredTableData" :key="'row-'+row.id" class="clickable-row">
                  ...
                  <td><span class="blue font-bold">{{ row.cycleTimeDays }} days</span></td>
                  <td class="emerald font-bold">{{ row.prsMerged }} PRs</td>
                  <td>
                    <div class="tbl-pct-cell">
                      <span>{{ row.qualityScore }}%</span>
                      <div class="progress-bar-track-xs">
                        <div
                          class="progress-bar-fill-sm emerald"
                          :style="{ width: row.qualityScore + '%' }"
                        ></div>
                      </div>
                    </div>
                  </td>
                  <td class="font-bold purple">{{ row.kpiScorePct }}%</td>
                  <td>
                    <span class="sc-status-pill grade-pill" :class="getGradeClass(row.grade)">{{ row.grade }}</span>
                  </td>
                </tr>
              </tbody>
            </table>
      ...
      <!-- ══════════════════════════════════════════════ -->
      <!-- SECTION 8 — AI Performance Intelligence       -->
      <!-- ══════════════════════════════════════════════ -->
      <section class="section-container">
        <div class="ai-intelligence-card">
          <div class="ai-header">
            <div class="ai-title-wrap">
              ...
              <h3 class="ai-title">AI Performance &amp; KPI Intelligence</h3>
            </div>
          ...
          <div class="ai-forecast-banner">
            <div class="afb-left">
              <span class="afb-label">AI Predicted Next Sprint Cycle Time</span>
              <div class="afb-val-row">
                <span class="afb-main-val">{{ aiIntelligence.forecast.expectedCycleTime }}</span>
                <span class="afb-conf-badge">Confidence: {{ aiIntelligence.forecast.confidenceScore }}%</span>
              </div>
            </div>

            <div class="afb-right-metrics">
              <div class="afb-sub"><span>Best Case:</span> <strong>{{ aiIntelligence.forecast.bestCaseCycleTime }}</strong></div>
              <div class="afb-sub"><span>Worst Case:</span> <strong>{{ aiIntelligence.forecast.worstCaseCycleTime }}</strong></div>
              <div class="afb-sub"><span>Predicted PRs Merged:</span> <strong>{{ aiIntelligence.forecast.predictedPrsNextSprint }} PRs</strong></div>
            </div>
          </div>
        </div>
      </section>
      ...
      <!-- ══════════════════════════════════════════════ -->
      <!-- SECTION 9 — KPI Statistics Summary Grid       -->
      <!-- ══════════════════════════════════════════════ -->
      <section class="section-container">
        <div class="section-header">
          <div class="sh-title-wrap">
            <h2 class="section-title">KPI Statistics Summary</h2>
            <span class="sec-subtitle">Aggregated statistical highlights across historical engineering cycles</span>
          </div>
        </div>

        <div class="stats-summary-grid">
          <div class="stat-mini-card"><span>Highest KPI Index</span><strong>{{ statsSummary.highestKpiIndex }}</strong></div>
          <div class="stat-mini-card"><span>Lowest KPI Index</span><strong>{{ statsSummary.lowestKpiIndex }}</strong></div>
          <div class="stat-mini-card"><span>Average KPI Score</span><strong>{{ statsSummary.averageKpiIndex }}</strong></div>
          <div class="stat-mini-card"><span>Median KPI Score</span><strong>{{ statsSummary.medianKpiIndex }}</strong></div>
          <div class="stat-mini-card"><span>Total PRs Merged</span><strong>{{ statsSummary.totalPrsMerged }}</strong></div>
          <div class="stat-mini-card"><span>Average Cycle Time</span><strong>{{ statsSummary.averageCycleTime }}</strong></div>
          <div class="stat-mini-card"><span>Average Quality Score</span><strong>{{ statsSummary.averageQualityScore }}</strong></div>
          <div class="stat-mini-card"><span>Review Speed</span><strong>{{ statsSummary.averageReviewTurnaround }}</strong></div>
          <div class="stat-mini-card"><span>Rework Ratio</span><strong>{{ statsSummary.workloadReworkRate }}</strong></div>
          <div class="stat-mini-card emerald"><span>Squad Rating</span><strong>{{ statsSummary.squadRatingGrade }}</strong></div>
        </div>
      </section>
      ...
      <section class="section-container">
        <div class="quick-actions-bar">
          <button class="qa-btn" @click="openJiraBoard">
    ...
    <div v-if="showReportModal" class="modal-backdrop" @click.self="showReportModal = false">
      <div class="modal-card-custom">
        <div class="modal-header">
          <h3>Team Performance KPI Report</h3>
    ...
    <div v-if="showShareModal" class="modal-backdrop" @click.self="showShareModal = false">
      <div class="modal-card-custom">
        <div class="modal-header">
          <h3>Share KPI Dashboard</h3>
          <button @click="showShareModal = false" class="btn-close">✕</button>
        </div>
        <div class="modal-body">
          <p>Copy sharable KPI dashboard link to email or Slack:</p>
...
useHead({ title: 'Team Performance KPIs | Sprintlytics' });
...
const roleOptions = computed(() => data.value?.roleOptions || [{ label: 'All Roles', value: 'ALL' }]);
...
const getGradeClass = (grade) => {
  if (!grade) return 'a-plus';
  const g = grade.toLowerCase();
  if (g.includes('a+')) return 'a-plus';
  if (g.includes('a')) return 'a-grade';
  return 'b-grade';
};

const getGranularTitle = (key) => {
  const titles = {
    leadTimeProduction: 'Lead Time to Production',
    prThroughput: 'PR Code Throughput',
    testPassRate: 'CI/CD Suite Pass Rate',
    firstPassApproval: 'PR First-Pass Approval',
    escapedDefects: 'Production Defect Leakage',
    resolutionTime: 'Blocker Resolution Speed',
    codeChurnVolume: '14-Day Code Churn Ratio',
    activeFocusTime: 'Developer Focus Hours'
  };
...
const fetchData = async () => {
  pending.value = true;
  try {
    const res = await $fetch('/api/team/kpis', {
      query: {
    ...
    if (res && res.success) {
      ...
      showToast(res.error || 'Failed to load KPI data', 'warn');
    }
  } catch (err) {
    console.error('Error loading team KPI analytics:', err);
    showToast('Failed to connect to team KPI endpoint', 'warn');
  } finally {
    pending.value = false;
  }
};
...
const kpiPillars = computed(() => data.value?.kpiPillars || []);
const cycleTimePhases = computed(() => data.value?.cycleTimePhases || []);
const granularKpis = computed(() => data.value?.granularKpis || {});
const members = computed(() => data.value?.members || []);
const kpiTrendHistory = computed(() => data.value?.kpiTrendHistory || []);
...
const target90Y = margin.top + (chartHeight - margin.top - margin.bottom) * (1 - 90 / 100);
const targetBandHeight = (chartHeight - margin.top - margin.bottom) * (10 / 100);

const trendPoints = computed(() => {
  const points = kpiTrendHistory.value;
  ...
  return points.map((pt, i) => {
    ...
    const predY = chartHeight - margin.bottom - (Math.min(pt.predictabilityPct, maxVal) / maxVal) * (chartHeight - margin.top - margin.bottom);
    const cycleY = chartHeight - margin.bottom - (Math.min(pt.cycleTimeDays * 25, maxVal) / maxVal) * (chartHeight - margin.top - margin.bottom);
    return {
      sprint: pt.sprint,
      predictabilityPct: pt.predictabilityPct,
      cycleTimeDays: pt.cycleTimeDays,
      prsMerged: pt.prsMerged,
      x,
      predY,
      cycleY
    };
...
const predictabilityLinePath = computed(() => {
  const pts = trendPoints.value;
  if (!pts.length) return '';
  return 'M ' + pts.map(p => `${p.x},${p.predY}`).join(' L ');
});

const predictabilityAreaPath = computed(() => {
  const pts = trendPoints.value;
  if (!pts.length) return '';
  const line = pts.map(p => `${p.x},${p.predY}`).join(' L ');
...
const cycleTimeLinePath = computed(() => {
  const pts = trendPoints.value;
  if (!pts.length) return '';
  return 'M ' + pts.map(p => `${p.x},${p.cycleY}`).join(' L ');
});
...
const filteredTableData = computed(() => {
  ...
  if (tableSearch.value.trim()) {
    ...
    list = list.filter(m =>
      ...
      String(m.cycleTimeDays).includes(q) ||
      String(m.prsMerged).includes(q) ||
      String(m.qualityScore).includes(q)
    );
  }

  if (tableRoleFilter.value !== 'ALL') {
    list = list.filter(m => m.role.toLowerCase() === tableRoleFilter.value.toLowerCase());
  }
...
const openJiraBoard = () => {
  showToast(`Opening Jira Board for ${selectedProject.value}...`, 'info');
...
const exportCSV = () => {
  const rows = [
    ['Engineer Name', 'Role', 'Company', 'Sprint', 'Cycle Time Days', 'PRs Merged', 'Quality Score Pct', 'KPI Score Pct', 'Grade Rating']
  ];
  members.value.forEach(m => {
    rows.push([m.name, m.role, m.companyName, m.sprintName, `${m.cycleTimeDays}d`, m.prsMerged, `${m.qualityScore}%`, `${m.kpiScorePct}%`, m.grade]);
  });
  ...
  link.setAttribute('download', `team_kpis_report_${selectedProject.value}.csv`);
...
.leg-color-dot.purple { background: #8B5CF6; }
.leg-color-dot.blue-dashed { background: #2563EB; }
.leg-color-dot.emerald-dashed { background: #059669; }
...
/* Cycle Time Breakdown Phase */
.phase-breakdown-wrapper {
  display: flex;
  flex-direction: column;
  gap: 1rem;
}
...
.wcc-name { font-size: 0.72rem; color: #4B5563; font-weight: 600; }
.wcc-body { display: flex; flex-direction: column; }
.wcc-pct { font-size: 0.95rem; color: #111827; font-weight: 700; }
.wcc-pts { font-size: 0.7rem; color: #6B7280; }
...
.grade-pill.a-plus { background: #ECFDF5; color: #059669; border: 1px solid #6EE7B7; }
.grade-pill.a-grade { background: #EFF6FF; color: #2563EB; border: 1px solid #93C5FD; }
.grade-pill.b-grade { background: #FFF7ED; color: #EA580C; border: 1px solid #FDBA74; }
...
.tc-stat .val.purple { color: #7C3AED; }
